...
namespace AlliedDefenses.UI
{
    ...
    public static class CommandText
    {
        ...
        private static string HowItWorksEn()
        {
            ...
            sb.AppendLine($"  {k} beacon  buy once (re-delivered free if lost). A heavy TWO-");
            sb.AppendLine("  HANDED prop: carry it out and set it down anywhere. Wherever it");
            sb.AppendLine("  sits it anchors the counter-play auras below.");
            sb.AppendLine("");
        ...
        private static string HowItWorksFr()
        {
            ...
            sb.AppendLine("");
            sb.AppendLine("BALISE DE DEFENSE (protection dehors, pres du vaisseau) :");
            sb.AppendLine($"  {k} beacon  achat unique (relivree gratuitement si perdue). Un");
            sb.AppendLine("  objet lourd A DEUX MAINS : porte-la dehors et pose-la ou tu veux.");
            sb.AppendLine("  La ou elle est, elle ancre les auras de contre-jeu ci-dessous.");
            sb.AppendLine("");
